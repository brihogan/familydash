...
- **Remaining:** redeploy the slimmed `trmnlService.js` to prod (rebuild container) so ongoing real pushes (<2KB) are accepted — until then the device shows the manually-pushed test snapshot. `sample-payload.json` left untracked.
