...
- **Remaining to finish:** (1) redeploy slimmed `trmnlService.js` to prod (rebuild container) so real pushes are accepted; (2) re-paste `server/trmnl/dashboard.liquid` (table version) into the TRMNL markup editor + Save. The slim test payload is already loaded in TRMNL, so the paste renders immediately. (CM6 editor injection via JS failed — clipboard API hangs without focus, execCommand insertText rejected — so the markup paste must be done by hand.) `sample-payload.json` left untracked.
