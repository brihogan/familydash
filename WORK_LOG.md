...
- **Deploy needed:** prod still runs the old `kids`-only `trmnlService.js`. Sequence: pull + rebuild/restart the app container on prod → open the FamilyDash dashboard once so `pushToTrmnl` fires the new `users` payload (5-min throttle) → then paste `server/trmnl/dashboard.liquid` into the TRMNL markup editor and Save. `server/trmnl/sample-payload.json` (real family data) left untracked.
