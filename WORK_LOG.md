...
- New `server/trmnl/dashboard.liquid` is the markup to paste into the TRMNL private-plugin editor (device is a **TRMNL X**, landscape, 16-gray). After several on-device rendering failures (vertical bar → black → horizontal line), root-caused two things: TRMNL's renderer chokes on a top-level `<style>` block and on CSS Grid `1fr` row sizing. Final markup uses **inline `style` attributes only, no `<style>`, no CSS grid** — a `flex-wrap` 3×2 of bordered cells (the "outline" separators), `conic-gradient` rings (no SVG), and framework `title`/`title_bar` classes. Mirrors the user's known-good template style.
- Drove the TRMNL editor via Claude-in-Chrome to diagnose the last "empty rectangle": the preview renders against the **last webhook payload, which is production data containing only `kids`** (real values, e.g. Daniel $53.08) — so the new `{% for u in users %}` loops over nothing. Also found the preview device was set to "TRMNL OG (2-bit)"; switched it to TRMNL X. Local DB has no `trmnl_webhook_url` (prod-only), confirming prod feeds the device.
- **THE real root cause (after prod rebuild still showed empty):** TRMNL's free tier **rejects webhook payloads >2KB** (HTTP 422 "Large payload received (4160 bytes), should be less than 2kb"). Every push of the rich `users` payload was silently dropped, so TRMNL kept the old `kids` data. Confirmed by POSTing directly to the webhook. **Fix:** slimmed `buildDashboardPayload` to ~1.5KB — dropped `kids`, `color`, `trophies`, tile `label`/`done`/`total`/`complete` (markup derives complete from `pct == 100`), and truncate `latest` to ~38 chars. Tiles now just `{emoji, pct}`. Verified: 1473 bytes, webhook accepts (200).
- **Markup is now a `<table>` (table-layout:fixed), not flexbox.** The framework `.layout` class forces `display:flex; justify-content:center`, which wrapped the flex grid into 2 ragged/masonry columns. A table gives a clean 3×2. Confirmed live via Claude-in-Chrome that the squares/rings/money/activity render correctly on the TRMNL X profile once data is present.
- **Live now in TRMNL** (pasted + saved via Claude-in-Chrome): clean 3×2 table grid renders with the slim test payload. Refinements applied per request — **full-height cells** (table + td `height:100%` + inner flex column), **activity pinned to the bottom** via `margin-top:auto` (flexible gap above), and a **subtle dotted divider** (`1px dotted #aaa`). Markup is now **pure ASCII** — literal glyphs are HTML numeric entities (`&#127903;` 🎟, `&#10003;` ✓, `&#10024;` ✨) because pasting UTF-8 through the CDP clipboard mangled multibyte chars; avatar/task emoji come from the data and are unaffected. Pasting worked via `pbcopy` + a real `cmd+v` (synthetic `execCommand`/`navigator.clipboard` both failed).
- **Real badge images** added: payload tiles now carry `img` (bare badge filename from `badges.image_file`, joined in the taskRows query) instead of emoji when a task set is badge-linked; the markup builds the public URL `https://dash.straychips.com/api/uploads/badges/<file>` and renders `<img class="image">` inside the ring (emoji/✓ fallback for chores + non-badge sets). Badge images are public (HTTP 200, no auth — confirmed via curl). Payload measured 1913 B for The Hogans (20 image tiles + 3 emoji). Added a **2KB size guard** in `buildDashboardPayload`: if a build exceeds ~2000 B it drops images (emoji fallback), then shortens activity — so a push can never silently 422.
- The TRMNL **web preview shows broken image icons** for the badges — that's expected: TRMNL fetches external images server-side at device-render time and the in-browser preview sandbox can't load them (per TRMNL help docs). Must verify legibility on the actual device (48px, dithered to 16-gray) — fall back to emoji if muddy.
- **Badge images were broken on-device → root cause: CORP header.** The app (helmet) sets `Cross-Origin-Resource-Policy: same-origin` on every response, so TRMNL's headless render worker couldn't embed `dash.straychips.com` badge `<img>`s (browser preview is more forgiving, hence broken there too but for sandbox reasons). Fix in `server/src/app.js`: middleware on `/api/uploads/badges` sets `Cross-Origin-Resource-Policy: cross-origin` (verified locally — badge route now cross-origin, other routes still same-origin). Per TRMNL docs, their worker also wants Content-Length/ETag/Last-Modified, which express.static already provides.
- **Bottom-pin made device-robust:** the `height:100%` chain pins activity to the bottom in the browser preview but not TRMNL's stricter render worker, so each cell's inner column now also carries `min-height:45vh` (viewport-relative, independent of the parent height chain).
- **Prod is on an OUTDATED commit:** real data flows (Sarah + real balances show), but tiles are emoji ⭐ (no `img`) and there's no CORP header → prod predates the badge-image work. **One redeploy to latest fixes both** (sends `img` + serves CORP). Stop manual webhook pushes now — prod is the live data source; my local pushes lack the second parent and would clobber real data. `sample-payload.json` left untracked.
- **9 badges per user (kids AND parents) via short-id + compact-array encoding.** No TRMNL+ (confirmed it doesn't lift the 2KB cap). Key insight: shrinking image *files* doesn't help — the payload only carries the reference, not the bytes; filename *length* is the cost. New `GET /api/uploads/badges/by-id/:id` route (app.js) resolves a badge id → image file (with CORP header), so the payload sends the numeric badge id instead of the long filename. Tiles are now compact arrays: `[badge_id, pct]` for badges, `[emoji, pct, 1]` for chores/non-badge (3rd slot = emoji flag). `MAX_TILES` 8→9 (3×3 grid), parents get tiles again (still hide money/tickets). Result: 6 members × 9 badges ≈ 1.4KB, comfortably under 2KB. Guard rewritten to trim activity then pop tiles from the fullest user. Markup reads `t[0]/t[1]/t[2]` and builds `/badges/by-id/{{ t[0] }}`. Verified: by-id route 200 + image/jpeg + CORP; payload 1139–1393B; render balanced.
- **TRMNL+ does NOT lift this plugin's 2KB webhook cap** — probed live: 2.6KB/3.6KB/5KB payloads all 422 with "should be less than 2kb. Subscribe to TRMNL+". With 6 members + badge images the payload busts 2KB, so the guard was dropping images (confirmed by the 24-char activity = guard's emergency trim). **Fix: only kids get tiles now** (`r.role !== 'parent'` in buildDashboardPayload; markup wraps tiles in `{% unless u.is_parent %}`). Parents already hide money/tickets, so they're just name + activity. Payload drops to 1589 B — images fit with headroom, guard won't fire. Redeploy prod to latest so it sends this <2KB shape AND the CORP header.
...
### 2026-06-05 — Pin shared task sets; compact grid for unpinned
- Matrix modal header gained a pin button (thumbtack) at the end of the title. Pinning sorts that task set to the top of `/tasks/shared`. DB v84 (+schema): `shared_task_set_pins` (family_id, kind 'badge'/'set', ref_id). Server: matrix endpoint returns `pin {kind,refId,pinned}`; new `POST /family/shared-pins`; shared-task-sets returns `pinned` + sorts pinned-first. Client: `familyApi.setSharedPin`; matrix `togglePin` (optimistic + onChanged refresh).
- SharedTaskSetsPage: pinned items render as full-width row cards at the top; unpinned render as compact CARDS (bg + border + shadow; medallion + name + avatar stack) in a `grid-cols-3 … lg:grid-cols-7` grid — 3 across on mobile, 7 on wide desktop (DOM-verified). Verified: pin/unpin toggles + reorders, test pin cleaned up, no console errors. Server change → prod needs image rebuild.

### 2026-06-05 — Fix: step-title click looked "done" if the leftmost kid was done
- Row-click used the leftmost cell as the representative, so `readOnly={focus.cell.done}` made the whole view read-only (no Mark complete, no toggles) whenever the first column was already done — even if others still needed it. Now: row-click read-only only when EVERY kid who has the step is done; the toggle list (`coUsersFor(..., includeDone)`) includes already-done kids rendered as a static green check ("Already completed") instead of a toggle, while not-done kids stay as toggles (default on). `allCoIds` only pre-selects not-done kids. Verified on r6 (Brian done/leftmost, Daniel not): active view, Mark complete enabled, Brian=check, Daniel=toggle. Client-only.
