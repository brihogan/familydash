 ...
 * no colored level outline. Two-pass:
 *   1. `-fuzz 6% -trim`  removes the white/shadow padding, auto-centering
 *      the badge in its frame (fixes the down-right offset of the source).
 *   2. `-crop 70%x70%`   center-crops to drop the colored outline ring.
 *
 * Idempotent: skips files whose smallest dimension is already <= 320px
 * (assumes 500x500 originals; cropped output is ~230x230).
 *
 * Usage:
 *   node server/scripts/cropBadgeImages.js [--dir=<dir>] [--percent=70]
 *
...
const files = readdirSync(targetDir).filter(f => /\.(jpe?g|png|gif|webp)$/i.test(f));
...
for (const file of files) {
  ...
  try {
    ...
    if (Math.min(w, h) <= 320) { skipped++; continue; }
