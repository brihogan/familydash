...
import { processRecurringRules } from './recurringRuleService.js';
...
/**
 * Build the per-member dashboard rows for a family: main-account balance,
 * ticket balance, today's chore progress, active task sets, trophy counts, and
 * the latest activity line.
 *
 * Extracted from routes/dashboard.js so both the web dashboard (JWT) and the
 * device endpoint (Garmin FamDash, device token) render byte-identical data.
 * Side-effect parity with a web dashboard load is intentional: recurring money
 * rules are processed and today's chore logs are lazily generated.
 *
 * @param {number} familyId
 * @returns {Array<object>} members
 */
export function buildFamilyDashboard(familyId) {
  processRecurringRules(familyId);
