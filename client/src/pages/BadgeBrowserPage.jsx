...
export default function BadgeBrowserPage() {
  ...
  // Active badge task set IDs for this user (to show "Active" chip)
  const [activeBadgeIds, setActiveBadgeIds] = useState(new Set());
  ...
  const handleEnrolled = (taskSetId) => {
    ...
    navigate(`/tasks/${targetId}/${taskSetId}`);
  };

  return (
    <div>
      {/* Header — matches Individual Pages pattern */}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {badges.map((badge) => {
            ...
            const optCount  = level ? (optCounts[level] ?? 0) : null;
