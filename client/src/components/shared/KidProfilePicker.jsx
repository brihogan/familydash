 ...
 * Optionally shows a gear icon for a "manage/everyone" mode, and a group icon
 * linking to a "shared" view (task sets 2+ members are doing).
 *
 * @param {{ kids: Array, currentId: string|number, routePrefix: string, manageRoute?: string, manageLabel?: string, sharedRoute?: string, sharedSelected?: boolean }} props
 */
export default function KidProfilePicker({ kids, currentId, routePrefix, manageRoute = null, manageLabel = 'Manage', sharedRoute = null, sharedSelected = false, className }) {
    ...
    <div className={className ?? "flex items-center gap-2 mb-5 p-1 overflow-x-auto scrollbar-hide"}>
      {sharedRoute && (
        <button
          ...
          title="Shared — task sets 2+ people are doing"
        >
